...
PPMIMAGE* rotateImage(PPMIMAGE *imgA) {
  ...
  for(int i=0; i<imgA->height; i++) {
    for(int j=0; j<imgA->width; j++) {
      imgB->mat[i][j].R = imgA->mat[imgA->height-1-i][imgA->width-1-j].R;
      imgB->mat[i][j].G = imgA->mat[imgA->height-1-i][imgA->width-1-j].G;
      imgB->mat[i][j].B = imgA->mat[imgA->height-1-i][imgA->width-1-j].B;
    }
  }
