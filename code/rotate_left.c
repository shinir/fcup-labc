...
PPMIMAGE* rotateImage(PPMIMAGE *imgA) {
  ...
  imgB->height = imgA->width;
  imgB->width = imgA->height;
  ...
  for(int i=0; i<imgA->width; i++) {
    for(int j=0; j<imgA->height; j++) {
      imgB->mat[i][j].R = imgA->mat[j][imgB->height-i-1].R;
      imgB->mat[i][j].G = imgA->mat[j][imgB->height-i-1].G;
      imgB->mat[i][j].B = imgA->mat[j][imgB->height-i-1].B;
    }
  }
  return imgB;
}
