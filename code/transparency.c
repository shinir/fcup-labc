...
*/
  ...
  for(int i=0; i<imgC->height; i++) {
    ...
    for(int j=0; j<imgC->width; j++) {
      if((i>=b && j>=a) && (y<imgA->height) && (x<imgA->width)) {
       aux = 1;
       imgC->mat[i][j].R = ( (alpha+imgA->mat[y][x].R) + ((1-alpha)*imgB->mat[i][j].R) )/2;
       imgC->mat[i][j].G = ( (alpha+imgA->mat[y][x].G) + ((1-alpha)*imgB->mat[i][j].G) )/2;
       imgC->mat[i][j].B = ( (alpha+imgA->mat[y][x].B) + ((1-alpha)*imgB->mat[i][j].B) )/2;
       x++;
      }
      else {
       imgC->mat[i][j].R = imgB->mat[i][j].R;
       imgC->mat[i][j].G = imgB->mat[i][j].G;
